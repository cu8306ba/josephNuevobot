...
async def users(client: Client, message: Message):	
	...
	if username in boss:	
		...
		for user in Configs:
			...
			if user == "gp":continue
			if user == "josephPg":continue
			if user == "Luhan2293":continue
			if user == "Lisva81":continue
			if user == "lilitah_99":continue
			if user == "bb0un_01":continue
			if user == "Damas99":continue
